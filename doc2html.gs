function generate_html_markup(id) {
  ...
  // createDocumentForHtml(id, html, images);
...
//function createDocumentForHtml(id, html, images) {
...
function cleanse_html(html) {
  html.toString();
  
  var stageA = html.replace(/(\s{2}\<p\>\<strong\>)(.*)(\<\/strong\>\<\/p\>\s)/g, function(matchedString, openTags, innerText, closeTags){
      openTags = '\n <h2>';
      closeTags = '</h2> \n';
      
      return openTags + innerText + closeTags;
  });
